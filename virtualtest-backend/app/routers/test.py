...
async def start_module(
    ...
    """
    Modülü başlatır ve AI içerik üretir.
    
    İşlem:
    1. Session kontrolü
    2. Modül zaten tamamlanmış mı kontrol
    3. AI'dan içerik üret
    4. Listening için ses dosyası oluştur
    """
    ...
    if module_name == "listening" and "script" in content:
        audio_path = tts_service.convert_text_to_audio(
            text=content["script"],
            slow=(cefr_level in ["A1", "A2"])
        )
        if audio_path:
            content["audio_url"] = tts_service.get_audio_url(audio_path)
